...
Bangladesh = cbind(tableToCorrelate[,c(1,2)], tableToCorrelate[,metadata$ID[metadata$site=="Uva"]]) 
corVals = correlate(Bangladesh, metadata$laz[metadata$site=="Uva"])
...
p + geom_point(color = "darkblue", size = 2) + facet_wrap(~Organism, scales = "free")+
...
a + geom_point(color = "darkblue") + theme(axis.text.x = element_text(hjust = 1, angle = 90)) + geom_hline(yintercept = 0, color = "gray60") +
  ggtitle("UVA") + ylab("Pearson correlation") + xlab(" ")
...
formula = y ~ x 
p = ggplot(corPlot,aes(laz, percent))
p + geom_point(color = "darkblue", size = 2) + facet_wrap(~genus, scales = "free")+
  ylab("Mapped proportions [%]") + xlab("LAZ") + ggtitle("UVA") +  
...
ggsave(filename = "figures/collapsedGenus_UVA.pdf", width = 16, height =9)
...
ggsave(filename = "figures/corValsUVA.pdf")


# plot only the interesting ones: 
