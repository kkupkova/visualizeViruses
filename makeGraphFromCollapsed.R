...
constructGraph = function(dataToPlot, 
  ...
  data = dataToPlot[apply(dataToPlot, 1, function(x) !all(x < threshold)),] 
  ...
  for(i in 1:numOTU){                     #for every taxon
    for(j in 1:numSamples){               #for every sample
      if(data[i,j]>0.001){         #if abundance of i-th taxon in j-th sample is higher that threshold
...
# run the function
graphList = constructGraph(as.data.frame(collapsedTable))
...
createNetworkFromIgraph(ig,"finalBestHit0_0005") 
